...
send_key_frame_request_to_sender() {
  GstPad *videosrcpad;
  videosrcpad = gst_element_get_static_pad(webrtc1, "src_0");
...
{
  GstPad *connector_pad;
  GstElement *q, *conv, *x264enc, *rtph264pay, *resample, *sink;
  ...
  if (g_strcmp0 (convert_name, "audioconvert") == 0) {
    ...
    connector_pad = gst_element_get_static_pad (q, "sink");
  } else {
    x264enc = gst_element_factory_make ("x264enc", NULL);
    g_object_set(G_OBJECT (x264enc),"tune","zerolatency", NULL);
    g_assert_nonnull (x264enc);
    ...
    g_assert_nonnull (rtph264pay);
    g_object_set(G_OBJECT (sink),"host","127.0.0.1", NULL);
    g_object_set(G_OBJECT (sink),"port",9500, NULL);
    g_object_set(G_OBJECT (sink),"sync",FALSE, NULL);
    g_object_set(G_OBJECT (sink),"async",FALSE, NULL);
    gst_bin_add_many (GST_BIN (pipe), x264enc, rtph264pay, sink, NULL);
    /* gst_bin_add_many (GST_BIN (pipe), x264enc, q, rtph264pay, sink, NULL); */
    gst_element_sync_state_with_parent (x264enc);
    /* gst_element_sync_state_with_parent (q); */
    gst_element_sync_state_with_parent (rtph264pay);
    gst_element_sync_state_with_parent (sink);
    gst_element_link_many ( x264enc, rtph264pay, sink, NULL);
    /* gst_element_link_many ( x264enc, q, rtph264pay, sink, NULL); */
    connector_pad = gst_element_get_static_pad (x264enc, "sink");
  }

  ret = gst_pad_link (pad, connector_pad);
    ...
    GstElement * pipe)
{
  ...
  if (g_str_has_prefix (name, "video")) {
    /* handle_media_stream_video_orig (pad, pipe); */
    /* handle_media_stream_video (pad, pipe); */
    /* handle_media_stream (pad, pipe, "videoconvert", "autovideosink"); */
    handle_media_stream (pad, pipe, "videoconvert", "udpsink");
  } else if (g_str_has_prefix (name, "audio")) {
    /* handle_media_stream_audio (pad, pipe); */
...
/* #define TURN_SERVER " turn-server=turn://rtaylor:password@api.anakodec.com:3478 " */
...
start_pipeline (void)
{
  ...
  /* pipe1 = */
  /*     gst_parse_launch ("webrtcbin bundle-policy=max-bundle name=sendrecv " TURN_SERVER */
  /*     "videotestsrc is-live=true pattern=ball ! videoconvert ! queue ! vp8enc deadline=1 ! rtpvp8pay ! " */
  /*     "queue ! " RTP_CAPS_VP8 "96 ! sendrecv. " */
  /*     "audiotestsrc is-live=true wave=red-noise ! audioconvert ! audioresample ! queue ! opusenc ! rtpopuspay ! " */
  /*     "queue ! " RTP_CAPS_OPUS "97 ! sendrecv. ", */
